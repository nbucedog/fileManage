...
DROP TABLE IF EXISTS `hoppo_file_info`;
create table `hoppo_file_info` (
`id` int(11) not null auto_increment,
`filename` longtext not null,
`filetype` varchar(40) not null,
`sharedurl` longtext not null,
`views` int not null,
`hoppo_products_id` varchar (95),
`brand` longtext,
`series` longtext,
`code` longtext not null,
primary key (`id`),
constraint fk_1 foreign key (hoppo_products_id) references hoppo_products (Id) on update cascade on delete restrict
)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;